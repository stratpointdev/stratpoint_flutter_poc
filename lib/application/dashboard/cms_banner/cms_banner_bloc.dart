...
class CmsBannerBloc extends Bloc<CmsBannerEvent, CmsBannerState> {
  ...
  Stream<CmsBannerState> mapEventToState(CmsBannerEvent event) async* {
    ...
    if (event is RefreshCmsBannerEvent || event is InitialCmsBannerEvent) {
      ...
      if (kIsWeb) {
        if (value.isRight()) {
          final List<Image> imageList = await cmsBannerRepository.getCmsBannerImage(value.getOrElse(() => null).getImagePathsForWeb());
          yield CmsBannerSuccessState(imageList: imageList,imageLinks: value.getOrElse(() => null).getImageLinks());
